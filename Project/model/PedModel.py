...
class PedModel(nn.Module):
    def __init__(self, args, A, frames=True, vel=False, seg=False, h3d=True, nodes=19, n_clss=1):
        ...
        # ----------------------------------------------------------------------------------------------------
        self.layer1 = GCN_TAT_unit(self.ch, self.ch1, args, A, residual=False)
        ...
        self.layer2 = GCN_TAT_unit(self.ch1, self.ch2, args, A)
        ...
        # ----------------------------------------------------------------------------------------------------
        #self.layers = nn.Linear(self.ch1, self.ch2)
...
class decoupling_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, args, A, adaptive=True):
...
class GCN_TAT_unit(nn.Module):
    def __init__(self, in_channels, out_channels, args, A, stride=1, residual=True, adaptive=True):
        ...
        #A = np.stack([np.eye(19)] * 3, axis=0)

        self.res = residual
        self.gcn = decoupling_gcn(in_channels, out_channels, args, A, adaptive)

        self.embed = Embedding_module(out_channels, self.feature_dims)
        self.tat = nn.ModuleList(
            Encoder(self.feature_dims, self.head_num, self.hidden_dims, self.rate) for _ in
            range(self.tat_num))
        self.relu = nn.ReLU(inplace=True)
        if not residual:
