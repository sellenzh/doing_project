...
class pedMondel(nn.Module):
    def __init__(self, frames, vel=False, seg=False, h3d=True, nodes=19, n_clss=1):
        ...
        self.img_width, self.height = 192, 64
        ...
        self.img0 = nn.Sequential(
            nn.Conv2d(self.ch, self.ch1, kernel_size=3, stride=1, bias=False),
            nn.BatchNorm2d(self.ch1), nn.SiLU()
        )
        self.vel0 = nn.Sequential(
            nn.Conv1d(2, self.ch1, bias=False),
        ...
        self.img1 = nn.Sequential(
            nn.Conv2d(self.ch1, self.ch1, kernel_size=3, bias=False),
            nn.BatchNorm2d(self.ch1), nn.SiLU()
        )
        ...
        self.img2 = nn.Sequential(
            nn.Conv2d(self.ch1, self.ch2, kernel_size=2, bias=False),
        ...
        self.pool_sig_2d1 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Sigmoid()
        )
        self.pool_sig_2d2 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Sigmoid()
        )
        self.pool_sig_1d1 = nn.Sequential(
            nn.AdaptiveAvgPool1d(1),
            nn.Sigmoid()
        )
        self.pool_sig_1d2 = nn.Sequential(
            nn.AdaptiveAvgPool1d(1),
            nn.Sigmoid()
        )
    ...
    def forward(self, pose, frame, vel):
        ...
        pose_bn = self.data_bn(pose)
        pose = pose_bn.view(N, C, V, T).permute(0, 1, 3, 2).contiguous()
        # -----------------------------------------------------------
        fr = self.img0(frame)
        ve = self.vel0(vel)
        pose1 = self.layer1(pose)
        # -----------------------------------------------------------
        fr1 = self.img1(fr)
        pose1.mul(self.pool_sig_2d1(fr1))
        ve1 = self.vel1(ve)
        pose1 = pose1.mul(self.pool_sig_1d1(ve1).unsqueeze(-1))
        pose2 = self.layer2(pose1)
        # -----------------------------------------------------------
        fr2 = self.img2(fr1)
        pose2 = pose2.mul(self.pool_sig_2d2(fr2))
        ve2 = self.vel2(ve1)
        pose2 = pose2.mul(self.pool_sig_1d2(ve2).unsqueeze(-1))
        # -----------------------------------------------------------
        po = self.gap(pose2).squeeze(-1)
        po = po.squeeze(-1)
        po = self.att(po).mul(po) + po
        y = self.drop(po)
        y = self.linear(y)
...
class decoupling_gcn(nn.Module):
    def __init__(self, in_ch, out_ch, A, adaptive=True):
        ...
        self.DecoupleA = nn.Parameter(torch.tensor(np.reshape(A.astype(np.float32),
                            [3, 1, 19, 19]), dtype=torch.float32, requires_grad=True).reshape(1, self.groups, 1, 1), requires_grad=True)
        if in_ch != out_ch:
            self.down = nn.Sequential(
                nn.Conv2d(self.in_ch, self.out_ch, 1),
        ...
        self.Linear_weight = nn.Parameter(torch.zeors(
            self.in_ch, self.out_ch * self.subnet, requires_grad=True, device='cuda'), requires_grad=True)
        nn.init.normal_(self.Linear_weight, 0, math.sqrt(
            0.5 / (self.out_ch * self.subnet)
        ))
        self.Linear_bias = nn.Parameter(torch.zeros(
            1, self.out_ch * self.subnet, 1, 1, requires_grad=True, device='cuda'), requires_grad=True)
    ...
    def forward(self, x):
        ...
        n, kc, t, v = y.size()
        y = y.view(n, self.subnet, kn // self.subnet, t, v)
        y = torch.einsum('nkctw,kcvw->nctw',(y, norm_learn_A))
