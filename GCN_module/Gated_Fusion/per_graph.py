...
from cross_model import CrossTransformer
...
class pedMondel(nn.Module):
    ...
    def __init__(self, frames, vel=False, seg=False, h3d=True, nodes=19, n_clss=1):
        ...
        if vel:
            ...
            self.vel0 = nn.Linear(2, self.ch1)
        ...
        if vel:
            ...
            self.vel1 = nn.Linear(self.ch1, self.ch2)
    ...
    def forward(self, kp, frame=None, vel=None):
        ...
        if self.vel:
            ...
            velocity = self.vel0(vel[:, :, -T:].permute(0, 2, 1)).permute(0, 2, 1)
        ...
        if self.vel:
            v1 = self.v2(v1)
            pose_mul = pose.mul(self.pool_sig_1d(v1).unsqueeze(-1))
            velocity = self.vel1(velocity.permute(0, 2, 1)).permute(0, 2, 1)
