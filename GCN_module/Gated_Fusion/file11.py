...
#cross model inputs:[B, C, T, V], [B, C, T]  outputs:[B, C, T, V]
...
class pedMondel(nn.Module):
    ...
    def __init__(self, frames, vel=False, seg=False, h3d=True, nodes=19, n_clss=1):
        ...
        if vel:
            ...
            self.vel1 = nn.Sequential(
                nn.Conv1d(2, self.ch1 * 3, kernel_size=1, stride=1, padding=0, bias=True),
                nn.BatchNorm1d(self.ch1 * 3), nn.SiLU(),
        ...
        if vel:
            ...
            self.cross1 = CrossTransformer(self.ch1)
            self.gated1 = GatedFusion(self.ch1)
        # ----------------------------------------------------------------------------------------------------
        ...
        if vel:
            ...
            '''self.vel2 = nn.Sequential(
                    nn.Linear(self.ch1, self.ch2*3), nn.ReLU(),
                    nn.Linear(self.ch2*3, self.ch2), nn.ReLU()
            )'''
            self.vel2 = nn.Sequential(
                nn.Conv1d(self.ch1, self.ch2 * 3, kernel_size=1, stride=1, padding=0, bias=True),
                nn.BatchNorm1d(self.ch2 * 3), nn.SiLU(),
                nn.Conv1d(self.ch2 * 3, self.ch2, kernel_size=1, stride=1, padding=0, bias=True),
    ...
    def forward(self, kp, frame=None, vel=None):
        ...
        if self.vel:
            ...
            pose = self.gated1(pose_mul, pose_att)
        ...
        if self.frames:
            f1 = self.conv2(f1)
            x1 = pose.mul(self.pool_sig_2d(f1))
