...
class DataSet(data.Dataset):
    ...
    def __init__(self, path, pie_path, data_set, frame, vel, bbox, 
            balance=False, bh='all', t23=False, transforms=None, seg_map=True,
            h3d=True, forecast=False, time_crop=False):
        np.random.seed(42)
        ...
        self.vid_ids, _ = imdb._get_data_ids(data_set, params)#return set_ids
    ...
    def __getitem__(self, item):
        ...
        #if self.bbox:
        #   bbox_x1 = np.asarray(ped_data['bbox']).reshape(1, -1) / 120.0

        if self.frame and not self.vel:
            return kp, bh, img
        elif self.frame and self.vel and self.bbox:
            return kp, bh, img, vel, bbox
        else:
            return kp, bh
