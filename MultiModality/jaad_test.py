...
from final_jaad_dataloader import DataSet
...
class musterModel(nn.Module):
    ...
    def forward(self, kps, img, bbox, vel):
        with torch.no_grad():
            cx, _, _, _ = self.model(kps, img, bbox, vel)
        return cx.unsqueeze(0).softmax(1)
...
def main(args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(data, desc='Testing samples')):
            ...
            f = batch[2].float().to(args.device)
            bbox = batch[3].float().to(args.device)
            v = batch[4].float().to(args.device)
            weather_ = batch[-2]
            models_data = batch[-1]
            
            str_t.record()
            pred = model(x.half(), f.half(), bbox.half(), v.half())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt', type=str, default="./log/JAAD/best.pth", help="Path to model weigths")
