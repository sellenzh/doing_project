...
class Model(nn.Module):
    def __init__(self, num_layers, d_model, bbox_input, speed_input, num_heads, dff, pe_input, pe_target, rate=0.1):
        ...
        #设置一个可学习的参数sigma
        self.sigma = nn.Parameter(torch.zeros(1, requires_grad=True, device=device), requires_grad=True)
        nn.init.constant_(self.sigma, 1e-6)
    ...
    def forward(self, data, inp_dec, combined_mask=None, dec_padding_mask=None, enc_padding_mask=None):
        ...
        y_traj = self.final_layer(self.end_point_time(traj))
        return y_traj, self.activation(self.dense(y)), self.sigma
