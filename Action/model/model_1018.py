...
class Encoder(nn.Module):
    def __init__(self, num_layers, d_model, bbox_input, speed_input, num_heads, dff, maximum_position_encoding, rate=0.1):
        ...
        self.conv_layers_bbox = nn.ModuleList()
        self.conv_layers_vel = nn.ModuleList()
        ...
        for _ in range(self.num_layers):
            ...
            self.conv_layers_bbox.append(nn.Conv1d(d_model, d_model, kernel_size=5, stride=1, padding=2))
            self.conv_layers_vel.append(nn.Conv1d(d_model, d_model, kernel_size=5, stride=1, padding=2))
        ...
        #self.gate3 = Gate(d_model)
        #self.gate4 = Gate(d_model)
        #self.gate1 = ConvGate(16)
        #self.gate2 = ConvGate(16)
    ...
    def forward(self, x, vel):
        ...
        for i in range(self.num_layers):
            ...
            sa_x = self.enc_layers[i](x)
            sa_vel = self.vel_layers[i](vel)
            bbox2vel = self.cross_bbox2vel[i](x, vel)
            vel2bbox = self.cross_vel2bbox[i](vel, x)
            #bbox_conv = self.conv_layers_bbox[i](x.transpose(-1, -2)).transpose(-1, -2)
            #vel_conv = self.conv_layers_vel[i](vel.transpose(-1, -2)).transpose(-1, -2)
            x = self.gate1(bbox2vel, sa_x)
            vel = self.gate2(vel2bbox, sa_vel)
            #x, vel = bbox2vel, vel2bbox
...
class Model(nn.Module):
    def __init__(self, num_layers, d_model, bbox_input, speed_input, num_heads, dff, maximum_position_encoding, rate=0.1):
        ...
        #self.gate = ConvGate(self.time_crop)

    def forward(self, data):
        #time_crop = random.randint(8, 16)
        #time_crop = self.time_crop #if random.randint(1, 10) > 2 else 12
        #x = data[:, -time_crop:, :-2]
        #vel = data[:, -time_crop:, -2:] 
        x, vel = data[:, :, :-2], data[:, :, -2:]
        x, vel = self.encoder(x, vel)
        #y = self.gate(x, vel)
        #y = torch.add(x, vel)# [32, 16, 128]
...
class Gate(nn.Module):
    def __init__(self, dims):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(dims, dims * 3), nn.Dropout(0.3), nn.LayerNorm(dims * 3), nn.ReLU(),
            nn.Linear(dims * 3, dims * 3), nn.Dropout(0.3), nn.LayerNorm(dims * 3), nn.ReLU(),
            nn.Linear(dims * 3, dims), nn.Dropout(0.3), nn.LayerNorm(dims), nn.ReLU(),
        )
...
class ConvGate(nn.Module):
    def __init__(self, time):
        ...
        self.fc1 = nn.Sequential(
            nn.Conv1d(time, time, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm1d(time), nn.ReLU()
        )
        self.fc2 = nn.Sequential(
            nn.Conv1d(time, time, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm1d(time), nn.ReLU()
