...
class Encoder(nn.Module):
    ...
    def forward(self, x, vel):
        ...
        for i in range(self.num_layers):
            ...
            bbox2vel = self.cross_bbox2vel[i](sa_x, sa_vel)
            vel2bbox = self.cross_vel2bbox[i](sa_vel, sa_x)
            #x = self.gate1(sa_x, bbox2vel)
            #vel = self.gate2(sa_vel, vel2bbox)
...
class Model(nn.Module):
    ...
    def forward(self, data):
        ...
        time_crop = 16 #if random.randint(1, 10) > 2 else 12
        x = data[:, -time_crop:, :-2]
