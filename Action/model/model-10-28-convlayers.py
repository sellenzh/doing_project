...
class ConvLayers(nn.Module):
    def __init__(self, dims):
        ...
        self.layers = nn.Sequential(
            nn.Conv1d(in_channels=dims, out_channels=dims * 2, kernel_size=7, stride=1, padding=3),
            nn.BatchNorm1d(dims * 2), nn.ReLU(),
            nn.Conv1d(in_channels=dims * 2, out_channels=dims * 2, kernel_size=5, stride=1, padding=2),
            nn.BatchNorm1d(dims * 2), nn.ReLU(),
            nn.Conv1d(in_channels=dims * 2, out_channels=dims, kernel_size=3, stride=1, padding=1),
