...
def train(model, train_loader, valid_loader, class_critirion, reg_critirion, cl_lambda, reg_lambda,
    ...
    for epoch in range(epochs):
        ...
        for x_enc, x_dec, y in train_loader:
            ...
            #thought sigma_cls and sigma_reg are squared parameters.
            ...
            f_loss = cl_loss / (2 * sigma_cls) + re_loss / sigma_reg + torch.log(torch.sqrt(sigma_cls)) + torch.log(torch.sqrt(sigma_reg))
...
def evaluate(model, data_loader, class_critirion, reg_critirion, cl_lambda, reg_lambda,):
    ...
    with torch.no_grad():
        ...
        for x_enc, x_dec, y in data_loader:
            ...
            val_f_loss = val_cl_loss / (2 * sigma_cls) + val_re_loss / sigma_reg + torch.log(torch.sqrt(sigma_cls)) + torch.log(torch.sqrt(sigma_reg))
