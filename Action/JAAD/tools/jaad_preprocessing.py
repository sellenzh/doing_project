...
def balance_dataset(dataset, flip = True):
    ...
    d = {'bbox': dataset['bbox'].copy(),
        ...
        #'kps': dataset['kps'].copy(),
         'image_dimension': (1920, 1080)}  
    
    #gt_labels = [gt[0] for gt in d['activities']]
    gt_labels = []
    for gt in d['activities']:
        if gt != []:
            gt_labels.append(gt[0])
    ...
    if num_neg_samples == num_pos_samples:
        ...
        for i in range(num_samples):
            if d['activities'][i] != [] and d['activities'][i][0][0] == gt_augment:
                ...
                #d['kps'].append(dataset['kps'][i].copy())
