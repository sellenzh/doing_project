...
from tools.jaad_data import JAAD
from tools.jaad_preprocessing import *
...
from model.model_jaad import Model
...
#obs_seq_train, traj_seq_train = tte_dataset(balanced_seq_train, input_opts['tte'], 0.8, 16)
obs_seq_train, traj_seq_train = tte_dataset(seq_train, input_opts['tte'], 0.8, 16)


seq_valid = imdb.generate_data_trajectory_sequence('val', **data_opts)
#balanced_seq_valid = balance_dataset(seq_valid)
#obs_seq_valid, traj_seq_valid = tte_dataset(balanced_seq_valid,  input_opts['tte'], 0, 16)
obs_seq_valid, traj_seq_valid = tte_dataset(seq_valid,  input_opts['tte'], 0, 16)


seq_test = imdb.generate_data_trajectory_sequence('test', **data_opts)
obs_seq_test, traj_seq_test = tte_dataset(seq_test, input_opts['tte'], 0, 16)
...
vel_train = np.array(obs_seq_train['vehicle_act'])#[..., 16, 1]
vel_valid = np.array(obs_seq_valid['vehicle_act'])
vel_test = np.array(obs_seq_test['vehicle_act'])
...
vel_train, vel_valid, vel_test = torch.Tensor(vel_train), torch.Tensor(vel_valid), torch.Tensor(vel_test)
