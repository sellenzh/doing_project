...
class Model(nn.Module):
    def __init__(self, num_layers, d_model, bbox_input, speed_input, num_heads, dff, pe_input, pe_target, rate=0.1):
        ...
        self.traj_weight1 = nn.Parameter(torch.zeros(
            pe_input, (pe_target - 1) * 3, d_model, requires_grad=True, device='cuda'), requires_grad=True)
        ...
        self.traj_bias = nn.Parameter(torch.zeros(
            1, (pe_target - 1) * 3, 1, requires_grad=True, device='cuda'), requires_grad=True)
        ...
        self.end_point_time = Time_att(d_model, time=59)
    ...
    def forward(self, data, inp_dec, combined_mask=None, dec_padding_mask=None, enc_padding_mask=None):
        ...
        for i in range(self.num_layers):
            ...
            traj1 = torch.einsum('bdc,duc->buc', (x, self.traj_weight1)).contiguous() + self.traj_bias
            traj += torch.einsum('buc,utc->btc', (traj1, self.traj_weight2)).contiguous()
        ...
        return self.final_layer(self.end_point_time(traj)), self.activation(self.dense(y))
