...
class Model(nn.Module):
    def __init__(self, args, rate=0.25):
        ...
        self.traj_resize = nn.Linear(d_model * 2, d_model)
        self.att_t = Time_att(d_model)

        self.final_layer = nn.Linear(d_model, bbox_input)
        self.linear = nn.Linear(d_model * 2, 4)
        self.relu = nn.ReLU()
        ...
        nn.init.kaiming_normal_(self.sigma_cls, mode='fan_in')
        ...
        nn.init.kaiming_normal_(self.sigma_reg, mode='fan_in')
    ...
    def forward(self, data, traj=None):
        ...
        y = self.att_t(y)
        y = self.relu(self.linear(torch.cat((y, self.endp2fea(endp)), dim=-1)))
