...
class DataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        temp_data = list(map(float, ped_data['bbox']))
        bbox = torch.from_numpy(np.array(temp_data)).float()
        ...
        return keypoints, bh, img, bbox, vel
...
def main():
    ...
    for i in iter_:
        pose, y, img, v = tr_data.__getitem__(i)
        fs[i] = img[0]
